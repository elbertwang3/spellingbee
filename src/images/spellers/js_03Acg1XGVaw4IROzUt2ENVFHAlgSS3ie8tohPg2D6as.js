...
(function($, Drupal) {
  Drupal.behaviors.beeTheme = {
    attach: function (context, settings) {
      ...
      var $button = $('button.button, a.button');
      var $circleBtn = $('.circle-arrow');
      var $modal = $('.modal');

      $navIcon.on('click', toggleSearch());

      $button.click(function() {
        $(this).toggleClass('pulse-reverse');
      });

      $circleBtn.click(function() {
        var self = this;
        //$(self).addClass('pulse-reverse-big');
        //afterAnimation($(self), 'pulse-reverse-big');
      });

      $modal.css('max-width', window.innerWidth);

      function toggleSearch() {
        $(this).toggleClass('open');
        $('.navbar-top-level').toggleClass('open');
      }

      function afterAnimation(el, animClass) {
        var self = el;
      ...
      try {
        var el = document.querySelectorAll(".page-name-about-the-bee .journey-introduction-block")[1];
        el.className += ' second';
      } catch(err) {}
...
;
(function($, Drupal, spellers, ages, states, grades) {
	Drupal.behaviors.meetTheSpellers = {
		attach: function (context, settings) {
			...
			new Vue({
				...
				data: function() {
					return {
                        ...
                        finalistVisibility: 'hidden',
                        showFilterValues: false,
                        finalistMap: {
                            semi_finalist: 'Finalists',
                            champ_finalist: 'Spellers Continuing in Finals',
				...
				computed: {
					gradeLevel: function() {
                        let i = this.modalSpeller.grade_level;
                        if (i === 0){
                            return 'Kindergarten';
                        }
                        let j = i % 10,
					...
					modalSpellerProfileFields: function() {
						console.log('towiefhuiwhef', this.modalSpeller);
						if (this.modalSpeller && this.modalSpeller.profile_fields) {
                            let fields = this.modalSpeller.profile_fields.split(",");
					...
					spellers: function() {
						let len = Object.keys(this.allFilters).length;
						if (this.allFilters.hasOwnProperty('finalist')) {
							len--;
						}

						if (Object.keys(this.allFilters).length) {
							let that = this;
							return this.allSpellers.filter(function(speller) {
								let cnt = 0;
								for (let filter in that.allFilters) {
									if (that.allFilters.hasOwnProperty(filter)) {
										if (speller[filter] === that.allFilters[filter]){
											cnt++;
										}
										if (filter === 'grade_level'
											&& speller[filter] === 0
											&& that.allFilters[filter] === 'K'){
											cnt++;
										}
									}
								}
							...
							return this.allSpellers.slice(0, this.page);
						}
					}
				},
				methods: {
					addFilter: function(type, value, element) {
						let filters = {};
						filters[type] = value;

						if (['semi_finalist', 'champ_finalist', 'final_ranking' ].indexOf(type) > -1) {
							this.removeFilter('finalist', 'finalistVisibility');
							filters['finalist'] = this.finalistMap[type];
						}
					...
					getLabel: function(field) {
						const labels = {
                            ...
                            'fav_hist_fig': "Favorite Historical Figure: ",
							'fav_movie': "Favorite Movie: ",
							'fav_music': "Favorite Music: ",
							'fav_musician': "Favorite Musician: ",
							'fav_song': "Favorite Song: ",
							'fav_sport': "Favorite Sport: ",
							'fav_sportsfig': "Favorite Sports Figure: ",
							'fav_subject': "Favorite Subject: ",
							'fav_tvshow': "Favorite TV Show: ",
							'fav_word': "Favorite Word: ",
							'previous_bees': ""
						};
						return labels[field];
					},

					setSpeller: function(speller, index) {
						this.modalSpeller = Object.assign({}, speller);
						this.index = index;
					},

					nextSpeller: function() {
						this.allFilters = Object.assign({}, this.allFilters);
						let len = this.spellers.length;

						if ((len - 1) === this.index)
						{
							this.index = -1;
						}
						this.modalSpeller = Object.assign({}, this.spellers[this.index + 1]);
						this.index = this.index + 1;
					},

					prevSpeller: function() {
						this.allFilters = Object.assign({}, this.allFilters);
						if (this.index === 0)
						{
							this.index = this.spellers.length;
						}
						this.modalSpeller = Object.assign({}, this.spellers[this.index - 1]);
						this.index = this.index - 1;
					},

					removeFilter: function(type, element){
						this[element] = 'hidden';
						let removedFilters = Object.assign({}, this.allFilters);
						if(type === 'finalist') {
							if (removedFilters.hasOwnProperty('semi_finalist')) {
								delete removedFilters['semi_finalist'];
							}
							if (removedFilters.hasOwnProperty('champ_finalist')) {
								delete removedFilters['champ_finalist'];
							}
							if (removedFilters.hasOwnProperty('final_ranking')) {
								delete removedFilters['final_ranking'];
							}
							delete removedFilters['finalist'];
						}
						else {
							delete removedFilters[type];
						}
						this.allFilters = Object.assign({}, removedFilters);
						this.showFilterValues = !!Object.keys(this.allFilters).length;
					}
				},
				/*
				* TODO: add setimeout or interval to add the spellers like the scroll function
				*
				* */
				created: function() {
					let that = this;
					$(window).scroll(function() {
						if ($(window).scrollTop() + $(window).height() > $(document).height() - 1000) {
							if (that.page < that.allSpellers.length) {
								that.page += 12;
							}
						}
					});
                    ...
                    if(GET['type'])
                    {
                        ...
                        this.addFilter(type, 1, 'finalistVisibility');
                    }
				}
			});
		}
	};
})(jQuery, Drupal, spellers, ages, states, grades);
    ...
    Drupal.behaviors.searchBar = {
        attach: function (context, settings) {
            ...
            new Vue({
                ...
                methods: {
                    toggleSearch: function() {
                        console.log('toggle',!this.showSearch);
            ...
            $('#search-button').on('click', function () {
                ...
                $('#search-box').one('webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend', function () {
                    $('#search-box').removeClass('slideInDown');
                });
            });

            $('#close-search-button').on('click', function () {
                $('#search-box').addClass('animated slideOutUp');
                $('#search-box').one('webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend', function () {
                    $('#search-box').removeClass('animated slideOutUp');
                });
            });
